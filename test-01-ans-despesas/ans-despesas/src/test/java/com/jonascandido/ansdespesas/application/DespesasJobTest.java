...
class DespesasJobTest {
    ...
    @Test
    void execute_deveBaixarExtrairFiltrarEConsolidarTodosOsArquivosEmUmUnicoCSV() throws Exception {
        ...
        Consolidator consolidator = mock(Consolidator.class);
        ...
        when(processor.filterDespesaFiles(anyList())).thenReturn(List.of(dummyExtractedFile));

        Path dummyFinalZip = Path.of("temp/consolidado_despesas.zip");
        when(consolidator.consolidate(anyList())).thenReturn(dummyFinalZip);

        DespesasJob job = new DespesasJob(client, downloader, selector, extractor, processor, consolidator);
        ...
        verify(processor, times(3)).filterDespesaFiles(anyList());

        verify(consolidator).consolidate(anyList());
    }
}
